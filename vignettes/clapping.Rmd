...
gsink <- run(function(it) for (i in it) NULL)
```

Here `run` is like `gen` but it runs immediately, collecting all
values, rather than making an iteror.  In this case, since `run` is
called with a function expression, `gsink` is a function that when
called will consume all items from the iterator `it`.
