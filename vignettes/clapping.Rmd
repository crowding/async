...
gsink <- function(it) run(for (i in it) NULL)
```

Here [run]() is like `gen` but it runs immediately, collecting all
values, rather than constructing an iteror. So `gsink` is a function
that when called will consume all items from the iterator `it`.
