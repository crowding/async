...
The piece is based on a rhythmic motif of handclaps counting out
groups of 3, 2, 1, 2, 3, 2, 1, 2, 3... with each group seperated by
one rest. In musical notion, this makes a 12-count loop:
...
If we were rewriting `print_pattern` to return a vector, we might
begin by allocating the vector A typical way to proceed would be to
rewrite `pattern` so that it allocates an output vector, whose size
would be calculated in advance, then place each note into the
appropriate place in the output. There would need to be another index
variable to track the output index, since the output index corresponds
to neither `c` nor `j`. (If you want to see that, skip forward to the
[appendix]().

But we can avoid dealing with that indexing by using a generator. To do this,
enclose the body of the function in a call to `gen()`, and change
each `cat()` to `yield()` to return the values.
...
Note also the appearance of an `itertools` call in the for-loop. In a
generator you can go ahead and put an iterator (or anything which has
an `iter` method) into the for-loop argument, which outside of a
generator would require use of the `foreach` package. So a for-loop is
running over an indefinitely long sequence.
...
haven't made any sounds yet, so let's address that with a
generator. `with_sound()` will take an iterator as input, and
constructs a new iterator which plays a sound for each nonzero number
it passes through.
...
`iter_map`. The other one is `sink` which just consumes all elements from
an iterator.
...
domain-specific language for iterators -- where the DSL is supposed to
look like the host language.
