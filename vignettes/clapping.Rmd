...
`yield()`. We can also move the outer loop into the `recycle` argument of an [iteror].
...
outer function returns. Meanwhile, fixing the number of `repeats`
ahead of time is no longer necessary; a generator can be in principle
infinite but it will only generate data as long as you keep requesting
more.
...
value, and saves its state to pause until the next call to `nextElemOr()`.

So we can get values out of our generator like this:

```{R}
p <- gen_pattern()
for (i in 1:23) {
  cat(nextElemOr(p, break))
}
cat("\n")
```
...
Here's a couple of utility functions that will come in handy. One is
an iteror equivalent of `lapply`, which I'll call `iapply`. The
other one is `isink` which just consumes and throws away all elements
from an iteror.
...
Generators and coroutines enjoy some syntax extensions over base R;
one is that use can use a regular `for` loop with an iteror. So we can
equivalently write `iapply` and `isink` using generator syntax.

```{R}
gapply <- function(it, f, ...) { list(it, f, ...)
  gen( for (x in it) yield(f(x, ...)) )
}

gsink <- function(it, then=invisible(NULL)) { force(it)
  gen({ for (i in it) NULL ; yield(then)}) |> nextElemOr(then, then)
}
```
