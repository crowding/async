...
* The underlying implementation now includes the back half of a compiler. As evidence of this, you can draw a graph of a coroutine's control structures by calling `drawGraph(gen)` (this requires you have Graphviz `dot` command installed on your system.)
...
As of `async` version 0.3 you can extract and visualize this graph with [drawGraph(g)][].
