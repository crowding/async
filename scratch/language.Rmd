...
The coroutine language itself has no notion of functions and will ignore anything written in a function definition.
...
We might implement the feature where function expressions are 
...
### Loops
 ...
 `for` loops in a coroutine can take iterators as well as plain vectors. The `in` argument to a `for` loop will be converted to the `iteror` interfaces using `iteror(x)`.
...
`repeat`, `while` and `for` otherwise work like in base R, including support `next` and `break` operators.

You can also write loops by using `switch()` with `goto()` as described below.

### Starting and stopping

 `stop(err)` will abort the present branch, saving the error message. It then jumps to the enclosing `tryCatch()` handler, if there is one, or to the first `on.exit` error handler, if any have been registered. A `tryCatch(error=)` handler will catch the error, After executing all `finally` and `on.exit` handlers, the coroutine propagates the error. An `async` propagates the error by rejecting the promise, while a `gen` rejects it as a regular R error.

 `return(val)` jumps  through all `finally` and `on.exit` handlers before returning the given value. For an `async` the promise is resolved with that value. For a generator, the value is discarded.

 It is possible to call `stop()` or `return()` from inside a `finally` or `on.exit` handler. In that case the new value and disposition overrides the old one;

### Pausing on input/output

 * `await(p)` in an async, pauses execution until the given promise resolves.
 * `yield(val)` in a generator, pauses execution, with that value being returned from the present call of `nextElemOr`
 * `yieldFrom(i)` takes an iterator as argument and yields successive values until it is exhausted, then continues. `yieldFrom(i)` is equivalent to writing `repeat yield(nextElemOr(i, break))`.
...
Short circuiting boolean operators `||` and `&&` will try first their left branch, which should return a logical. If not satisfied, they will evaluate the right argument and return it directly. That is, in a coroutine `yield(TRUE) && "string"` may return "string", while in base R this raises an error.

I write "may" above, because if you write `||` in a generator expression you might be using either base R or coroutine version of `||`. In general you will only be using coroutine `||` if there is a pause somewhere on either side of it. So if you write `(x == 5) || stop("warning")` you will be using base R `||`, whereas if you write `(await(success) == 5) || stop("warning")` you will be using the coroutine `||` because there is an `await` on one side of it.

#### `switch` and `goto`

`switch()` statements are more strict than in base R; the input to a `switch` _must_ match a given branch or it will produce an error. That is, in base R, `switch(-1, "not run")` or `switch("bar", foo="not run")` returns invisible `NULL`, while in a coroutine they will throw an error.

Additionally, `switch` statements support a _delimited goto_. Within a given `switch` statement, `goto("other_branch")` stops executing the present branch and jumps to the given sibling branch. `goto()` without arguments re-reads the switch input again.
...
If  `goto` appears inside of a `try(..., finally={...})` call, which is itself inside a branch, the `finally` clause will be executed _before_ jumping to the new branch.

### Exceptions and cleanup

Writing code in a coroutine's `on.exit` clause or in a `tryCatch(finally=)` call does not give any guarantee that the cleanup code will ever run, because coroutines only execute "on demand." If you make a generator designed to return ten items, then call `nextElemOr` ten times, the generator will never run its exit handler. The generator is paused waiting to reach end of iteration; you need to call `nextElemOr` an _eleventh_ time to allow it to reach the end, runi ts exit handlers and signal end of iteration.
 ...
 * `on.exit` handlers can be registered dynamically. Whatever is written in the `expr` argument of `on.exit` is notionally "outside" any loops or other scope.
   ...
   * In a generator `gen(tryCatch({doStuff(); yield(x), finally=cleanup()}))`, the first call to the iterator's `nextElemOr` returns `x`. The `finally` clause will only be executed on the _next_ call to `nextElemOr`.
   * A difference from base R `tryCatch`: If you don't care about the type of error, you can specify a non-function value to `error=`, and that value will be returned simply. Additionally `error` can take flow control constructs like `break` or `next` or `goto`.
    * `stop()` will jump to the enclosing `tryCatch()` handler if there is one. Otherwise, `stop` in an `async` stops processing and rejects the promise; `stop` in a generator stops processing and terminates the generator (propagating the error).
